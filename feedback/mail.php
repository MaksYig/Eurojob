	...
	if($countFiles <= COUNT_FILES){
        ...
        for ($i = 0; $i < $countFiles; $i++) {
            ...
            if (!in_array($fileExtension, $allowedExtensions)) {
                $json['result'] = 'error';
                $json['error'] .= 'File type "' . $fileName . '" can only be DOC or PDF';
            }
            ...
            $json['result'] = 'error';
            $json['error'] = 'You can attache only 2 files ' . COUNT_FILES;
        }
...
if ($json['result'] == 'success') {
    ...
    try {
        ...
        $mail->Body    = $message;
        $mail->AltBody = 'Open this mail in Browser';
        ...
        $json['result'] = 'error';
        $json['error'] = 'Sending error' . $mail->ErrorInfo;
    }
...
if ($json['result'] == 'success' && !$user_email=='' && MAIL_CLIENT) {
    ...
    try {
        ...
        $mail_client->AltBody = 'Open this mail in browser';
